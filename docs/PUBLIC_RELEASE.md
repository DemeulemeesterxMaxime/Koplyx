...
- Lancer `koplyx --hidden` sans hote d'indicateurs, verifier que Koplyx reste accessible par raccourci global, puis que la croix masque la fenetre sans arreter le processus.
...
```bash
snapcraft pack
ls -lh koplyx_0.4.3_amd64.snap
snapcraft upload --release=stable koplyx_0.4.3_amd64.snap
```
