...
Au premier lancement, l'assistant de collage configure le raccourci GNOME, affiche le diagnostic de session puis exécute un test actif dans le champ que vous choisissez. Le test est exclu de l'historique et le presse-papiers précédent est restauré lorsque le bureau le permet. L'assistant reste relançable depuis Paramètres, avec un choix explicite entre `wtype`, XWayland local, Xorg, `ydotool`, le portail ou le mode presse-papiers uniquement. Aucun privilège n'est demandé sans clic de votre part.
